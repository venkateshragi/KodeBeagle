...
object MethodVisitorHelper {
  ...
  Map[String, Map[String, List[Int]]] =
    {
      val imports = parser.getImportDeclMap.values
      ...
      importWithMethodAndLineNumbers.filter {
        case (importName, methodAndLineNumbers) => imports.contains(importName)
      }
    }
  ...
  Map[String, Map[String, List[Int]]] =
    {
      ...
      (javaHashMap map {
        case (k, v) => k -> v.toMap.map { case (k, v) => k -> v.toList.map(_.toInt) }
      }).toMap
